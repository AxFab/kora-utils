...
opt_t options[] = {
    END_OPTION("Compute the CRC32 checksum of FILE, or standard input, to standard output.\n"
...
{
    checksum = CRC32_T[(checksum ^ cin) & 0xFF] ^ (checksum >> 8);
}
...
int convert_file(int fd, const char *name)
{
    int i;
    char buf[BUF_SZ];
    for (;;) {
